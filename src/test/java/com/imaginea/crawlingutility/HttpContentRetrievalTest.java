...
public class HttpContentRetrievalTest {
	...
	/**
	 * Test method for
	 * {@link com.imaginea.crawlingutility.HttpContentRetrieval#getHttpPageContent(java.lang.String)}
	 * .
	 */
	@Test
	public void testGetHttpPageContent_inCorrectUrl() {
		...
		try {
			httpContentRetrieval.setHttpContentRetrievalParameters(
	...
	public void testGetHttpPageContent_correctUrl() {
		...
		try {
			httpContentRetrieval.setHttpContentRetrievalParameters(
					connectionRetryCount, waitTimeout);
			httpContentRetrieved = httpContentRetrieval.getHttpPageContent(url);
			assertFalse("Result: " + testCaseType + " : ",
					httpContentRetrieved.isEmpty());
			System.out.println("Content: " + httpContentRetrieved);
		} catch (MalformedURLException e) {
			e.printStackTrace();
