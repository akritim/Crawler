...
public class HttpFileDownloaderTest {
	...
	/**
	 * Test method for
	 * {@link com.imaginea.downloadutility.HttpFileDownloader#run()}.
	 */
	@Test
	public void testRun_NoChangeInSharedQueue() {
		...
		httpFileDownloader1.setHttpFileDownloaderParameters(saveDirectory,
				sharedQueue, connectionRetryCount, waitTimeout);
		httpFileDownloader1.run();
		int finalQueueSize = sharedQueue.size();
		assertTrue("Result: " + testCaseType + " : ",
				finalQueueSize == initialQueueSize);
	...
	public void testRun_SharedQueueHasData_isParserRunningFalse() {
		...
		httpFileDownloader2.setHttpFileDownloaderParameters(saveDirectory,
				sharedQueue, connectionRetryCount, waitTimeout);
		httpFileDownloader2.run();
		int finalQueueSize = sharedQueue.size();
		assertTrue("Result: " + testCaseType
				+ " : SharedQueueHasData_isParserRunningFalse",
				finalQueueSize < initialQueueSize);
		System.out.println(testCaseType);
		System.out.println("Initial Queue Size: " + initialQueueSize);
		System.out.println("Final Queue Size: " + finalQueueSize);
	...
	public void testRun_incorrectMailLink() {
		...
		httpFileDownloader3.setHttpFileDownloaderParameters(saveDirectory,
				sharedQueue, connectionRetryCount, waitTimeout);
		try {
			sharedQueue.put(mailLink);
		} catch (InterruptedException e) {
